...
vec4 colorDiffuse(const vec4 &matColor, vec4 triangle[], vec4 normal_ends[], const mat4 &transform, Light *light)
{
...
vec4 colorSpecular(const vec4 &matColor, const float &shininess, const vec4 &camLocation, vec4 triangle[], vec4 normal_ends[], const mat4 &transform, Light *light)
{
    vec4 loc = (triangle[0]+triangle[1]+triangle[2])/3.0;
    vec4 l = -1.0f*transform* light->getSource()+loc;
    vec4 n = (normal_ends[0]+normal_ends[1]+normal_ends[2])/3.0;
