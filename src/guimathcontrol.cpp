...
mat2 sl2::rotate(float angle){
    mat2 j = mat2(vec2(cos(angle*M_PI), -sin(angle*M_PI)),
                  vec2(sin(angle*M_PI), cos(angle*M_PI)));
    return  j;
}
...
bool GUIMathControl::sl2matrix(float& s, SL2type sl2type, int id=1){
        switch(sl2type){
                ...
                case(ROTATE):
                            ...
                            ImGui::SameLine();
                            ImGui::Text("%.2f", cos(s));
                            if(ImGui::SliderFloat("##float_rotate", &s, 0.0, 3.14)) {ImGui::PopID(); return true;}
                            ImGui::PopID();
                            break;
