...
inline bool isClose(const float& a, const float& b){
    return std::abs(a-b) <= std::numeric_limits<float>::epsilon();
}

inline bool isRelClose(const float& a, const float& b){
    return std::abs(a-b) <= std::max(std::abs(a),std::abs(b))
                           *std::numeric_limits<float>::epsilon();
}

inline bool isRelLess(const float& a, const float& b){
...
vec3 normalize( const vec3& v ) {
    float l = length(v);
    if(l>EPSILON_FLOAT) return v / l;
    else return v;
}

inline
void forgetSize( vec3& v ) {
    float l = length(v);
...
inline
bool isRelClose(const vec3& v, const vec3& w){
    return isRelClose(v.x,w.x) &&
           isRelClose(v.y,w.y) &&
           isRelClose(v.z,w.z);
}
inline
...
inline
bool lessRelVector(const vec3& v, const vec3& w){
    if(isRelClose(v,w)) return false;
    else return isRelLess(v.x,w.x)
            ||  (isRelClose(v.x,w.x) && isRelLess(v.y,w.y))
            ||  (isRelClose(v.x,w.x) && isRelClose(v.y,w.y) && isRelLess(v.z,w.z));
}
...
inline
vec3 dehomogenize(const vec4& a){
    return vec3(a.x,a.y,a.z)/a.w;
}
